          ...
          const hitsOf = function(T){
            ...
            NATAL.forEach(function(pair){
              ...
              TYPES.forEach(function(t){
                ...
                if(orb <= t[2]) out.push({ point: pair[0], sign: SIGNOF[pair[0]] || '', house: HOUSEOF[pair[0]] || '', type: t[1], orb: Math.round(orb * 10) / 10, cr: (pair[0] === crName || (crCo && pair[0] === crCo)), crM: !!(crCo && pair[0] === crCo) });
              });
            });
...
Wichtig: Ich arbeite mit Placidus-Häusern und lese mein Horoskop über die Häuserherrscher. Zu jedem relevanten Haus stehen unten das Zeichen auf der Häuserspitze und der Herrscher mit seiner Position in meiner Radix. Lies jede Finsternis über ihr Haus UND über den Herrscher dieses Hauses: Das Haus zeigt, wo es geschieht, sein Herrscher zeigt, worüber es läuft und wohin es führt. Bei den Aspekten gilt: Ich arbeite bei Finsternissen mit einem weiten Orb von bis zu 10°. Je enger der Orb, desto stärker wirkt der Aspekt, die engsten tragen die Hauptbotschaft. Aspekte mit dem Vermerk HOROSKOPHERRSCHER treffen den Herrscher meines 1. Hauses, den Planeten, der mein ganzes Horoskop lenkt. Hat mein Aszendent-Zeichen zwei Herrscher, gilt der Vermerk für beide: Treffer auf den Mitherrscher sind mit HOROSKOPHERRSCHER, Mitherrscher gekennzeichnet und zählen genauso, das ist kein Widerspruch. Wenn du dich bei einer Deutung zwischen den beiden Herrschern entscheiden musst, nimm den modernen Herrscher: für Skorpion Pluto, für Wassermann Uranus, für Fische Neptun. Das gilt genauso für die Häuserherrscher. Solche Treffer wiegen doppelt.
...
Hier sind meine Daten:`;
  ...
  window.copyEclipseReading = function(){
    ...
    if(cr && crHit){
      chap.push('Der Treffer auf meinen Horoskopherrscher. Der Herrscher meines 1. Hauses ist ' + cr.name + (cr.sign ? (', er steht in ' + cr.sign) : '') + (cr.house ? (' in meinem ' + cr.house + '. Haus') : '') + ' und lenkt mein ganzes Horoskop.' + (cr.co ? (' Mein Aszendent-Zeichen hat zwei Herrscher: Mitherrscher ist ' + cr.co + ', Treffer auf ihn tragen den Vermerk HOROSKOPHERRSCHER, Mitherrscher und zählen genauso.') : '') + ' Eine Finsternis berührt diesen Bereich direkt, du findest jeden solchen Aspekt in den Daten am Vermerk HOROSKOPHERRSCHER. Widme diesem Treffer einen eigenen Absatz: Wenn der Lenker des Horoskops getroffen wird, verschiebt sich die Richtung des ganzen Lebens spürbarer als bei jedem anderen Punkt.');
    }
    ...
    const hitLine = function(h, who){ return '- ' + who + ' ' + h.type + ' zu ' + h.point + (h.sign ? (' in ' + h.sign) : '') + (h.house ? (', ' + h.house + '. Haus') : '') + ' (Orb ' + String(h.orb).replace('.', ',') + '°)' + (h.cr ? (h.crM ? ' [HOROSKOPHERRSCHER, Mitherrscher]' : ' [HOROSKOPHERRSCHER]') : ''); };
    if((sofi.hits || []).length){
